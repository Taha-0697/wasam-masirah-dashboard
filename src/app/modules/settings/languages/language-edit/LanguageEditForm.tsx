...
import { Language, LanguageRequest } from '../core/_models';
import { updateLanguage } from '../core/_requests';
...
const LanguageEditForm: FC<Props> = ({ language }) => {
  const [editLanguage] = useState<LanguageRequest>({
    name: language.name,
    abbreviation: language.abbreviation,
    direction: language.direction,
    iconPath: language.icon,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        values = {
          ...values,
          iconPath: iconRemoved ? '' : values.iconPath,
        };
  ...
  const getIconImage = () => {
    ...
    if (uploadedIcon) {
      image = uploadedIcon;
    } else if (language.icon && !iconRemoved) {
      image = `${process.env.REACT_APP_PUBLIC_URL}/images/${language.icon}`;
    }
  ...
  const onIconImageUpload = (event) => {
    ...
    formik.setFieldValue('uploadedIconFile', event.currentTarget.files[0]);
  };
      ...
      <KTCardBody>
        <div className='row g-2'>
          ...
          <div className='col-md-12'>
            {/* begin::Label */}
            <label className='form-label'>
              <FormattedMessage id='FIELDS.MESSAGE' />
