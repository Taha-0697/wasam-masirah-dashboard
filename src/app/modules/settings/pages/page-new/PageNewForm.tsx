...
import { PageRequest } from '../core/_models';
...
import { createPage, getPages, getSystemPages } from '../core/_requests';
...
const PageNewForm: FC<Props> = ({ page }) => {
  const [newPage] = useState<PageRequest>({
    name: page.name,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        backtoPageList();
        showToast('success', 'Page Added Successfully');
      } catch (error) {
    ...
    <form onSubmit={formik.handleSubmit} noValidate autoComplete='off'>
      <div className='card-header'>
        <div className='card-title'>
          <h3>
            <FormattedMessage id='ACTIONS.NEW' /> Page
          </h3>
        </div>
        ...
        <div className='row g-2'>
          ...
          <div className='col-md-12'>
            <label className='form-label'>
              ...
              <span className='required'></span>
              <InfoToolTip tooltipContent='Description of the Page' />
            </label>
            <textarea
