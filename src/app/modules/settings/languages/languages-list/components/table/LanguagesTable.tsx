...
import { languagesColumns } from './columns/_columns';
import { Language } from '../../../core/_models';
import { LanguagesListLoading } from '../loading/LanguagesListLoading';
import { LanguagesListPagination } from '../pagination/LanguagesListPagination';
import { useQueryResponseData, useQueryResponseLoading } from '../../../core/QueryResponseProvider';
import { LanguageCustomHeader } from './columns/LanguageCustomHeader';
import { LanguageActionsCell } from './columns/LanguageActionsCell';
...
const LanguagesTable = () => {
  ...
  const columns = useMemo(() => {
    ...
    const clonedColumns = [...languagesColumns];
    ...
    <>
      <div className='table-responsive'>
        <table className='table table-bordered align-middle text-dark'>
          ...
          <tbody>
            {data.length > 0 ? (
              getRowModel().rows.map((row: Row<Language>) => <CustomRow row={row} key={row.id} />)
            ) : (
              <tr>
                <td colSpan={7}>
